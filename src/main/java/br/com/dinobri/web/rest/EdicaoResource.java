...
/**
 * REST controller for managing {@link br.com.dinobri.domain.Edicao}.
 */
@RestController
@RequestMapping("/api")
@Transactional
public class EdicaoResource {
    ...
    public EdicaoResource(EdicaoRepository edicaoRepository) {
    ...
    public ResponseEntity<Edicao> createEdicao(@RequestBody Edicao edicao) throws URISyntaxException {
        ...
        Edicao result = edicaoRepository.save(edicao);
        return ResponseEntity.created(new URI("/api/edicaos/" + result.getId()))
            .headers(HeaderUtil.createEntityCreationAlert(applicationName, true, ENTITY_NAME, result.getId().toString()))
            .body(result);
    }

    /**
     * {@code PUT  /edicaos} : Updates an existing edicao.
     *
     ...
     */
    @PutMapping("/edicaos")
    public ResponseEntity<Edicao> updateEdicao(@RequestBody Edicao edicao) throws URISyntaxException {
        log.debug("REST request to update Edicao : {}", edicao);
        if (edicao.getId() == null) {
     ...
     */
    @GetMapping("/edicaos")
    public List<Edicao> getAllEdicaos() {
        log.debug("REST request to get all Edicaos");
